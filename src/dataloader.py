...
class RandomWindowSeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
            ...
            #hist_range = torch.norm(hist_pc, dim=1, keepdim=True)  # [T,1,H,W]

            # invalid mask for hist: xyz == 0 in all 3 channels
            #hist_mask = (hist_pc == 0).all(dim=1, keepdim=True)    # [T,1,H,W]
            #hist_range = hist_range.masked_fill(hist_mask.squeeze(1), -1.0).unsqueeze(1)  # careful with dims

            hist_range = torch.norm(hist_pc, dim=1, keepdim=True)          # [T,1,H,W]
            hist_mask  = (hist_pc == 0).all(dim=1, keepdim=True)           # [T,1,H,W]
            hist_range = hist_range.masked_fill(hist_mask, -1.0)           # [T,1,H,W]
            hist_xyzd  = torch.cat([hist_pc, hist_range], dim=1)           # [T,4,H,W]
