...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        print(f"The Spatial Attention Input {x.shape}")
        ...
        if self.attn_shortcut:
            x = x + shortcut
        print(f"The Spatial Attention Output {x.shape}")
        return x
...
class TemporalAttention(nn.Module):
    ...
    def forward(self, x):
        if self.attn_shortcut:
            shortcut = x.clone()
        x = self.proj_1(x) ## (2,80,16,512) || (2,256,16,512)
        x = self.activation(x)
        x = self.spatial_gating_unit(x) ## (2,80,16,512) || (2,256,16,512)
        x = self.proj_2(x) ## (2,80,16,512) || (2,256,16,512)
        if self.attn_shortcut:
            x = x + shortcut
        return x
...
class TemporalAttentionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        se_atten = self.fc(se_atten).view(b, c, 1, 1) ## (2,80,1,1) or (2,256,1,1)
        #print(f"se atten {se_atten.shape}")
        #print(f"f_x {f_x.shape}")
