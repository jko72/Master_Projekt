...
class projection:
    ...
    def __init__(self, cfg):
        ...
        fov_up = (
            self.cfg["model_params"]["FOV_UP"] / 180.0 * np.pi
        )  # field of view up in radians
        fov_down = (
            self.cfg["model_params"]["FOV_DOWN"] / 180.0 * np.pi
        )  # field of view down in radians
        fov = abs(fov_down) + abs(fov_up)  # get field of view total in radian
        W = self.cfg["model_params"]["grid_width"]
        H = self.cfg["model_params"]["grid_height"]
    ...
    def get_masked_range_view(self, output):
        ...
        masked_range_view[mask < self.cfg["model_params"]["MASK_THRESHOLD"]] = -1.0
